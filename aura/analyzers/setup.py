...
class SetupPy(NodeAnalyzerV2):
    ...
    def __parse_setup(self, node:Call):
        ...
        for pkg in pkgs:
            ...
            if isinstance(parsed.get('name'), str) and not self.__check_name(parsed['name'], pkg):
                sig = SetupScript(
                    score=100,
    ...
    def post_analysis(self, analyzer):
        ...
        for x in analyzer.hits:
            if isinstance(x, rules.FunctionCall) and 'code_execution' in x.tags:
                sig = SetupScript(
                    score=100,
            ...
            elif isinstance(x, rules.ModuleImport) and 'network' in x.categories:
                sig = SetupScript(
                    score = 100,
    ...
    def __parse_cmdclass(self, node):
        ...
        if isinstance(node.kwargs['cmdclass'], Dictionary):
            parsed['install_hooks'] = [self.__as_str(x) for x in node.kwargs['cmdclass'].keys]
